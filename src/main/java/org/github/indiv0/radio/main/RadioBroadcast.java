...
public class RadioBroadcast implements Runnable {
    ...
    public void attemptBroadcast(final Radio radio) {
        ...
        // Checks how many IRON_FENCE blocks there are above the structure, capping the number at 16. IRON_FENCE blocks determine the range of the
        // signal.
        int ironBarCount;
        ...
        final double broadcastDistance = ironBarCount * ironBarExtension + broadcastClarity * 550;
    ...
    public void broadcast(final Radio radio, final double percent, final boolean isGarbled, final Player player) {
        ...
        String freqPrefix = ChatColor.RED + "[Radio " + radio.getFrequency().getFrequency() + "] " + ChatColor.WHITE;

        ArrayList<String> message = Radio.getMessage(radio.getLocation());

        // Cancels the broadcast if there is no message provided.
        if (message.size() == 0 && transmitEmptyMessages) {
